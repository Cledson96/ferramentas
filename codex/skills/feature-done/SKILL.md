...
Quando o usuario executar `$feature-done`, siga este fluxo:
...
- se houver TASK-ID na branch, usar `getAccessibleAtlassianResources` + `getJiraIssue` para buscar `summary`, `description`, `issuetype.name`, `status.name` e `priority.name`

Se o diff estiver vazio, avisar que nao ha diferencas em relacao a `{BASE}` e encerrar.

### Fase 1/4 - Review com `$qa-agent`

Invocar o `$qa-agent` com o contexto ja coletado.
...
Se o usuario quiser seguir com docs, delegar o trabalho para `$confluence-docs`, repassando:
- diff da branch
- contexto Jira
...
Depois disso, usar o fluxo da skill `$commit` para:
...
- nao reimplementar dentro desta skill o fluxo que ja pertence a `$confluence-docs`, `$commit` ou `$pull-request`
