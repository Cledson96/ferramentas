---
name: jc-docs
description: "Gera documentacao tecnica em docs/ e sincroniza com Confluence (espaco Tech) via REST API com Storage Format XHTML. Usar apos implementar uma feature.. Use quando o usuario mencionar /jc:docs, pedir esse workflow da JusCash ou quiser o comportamento equivalente do plugin jc no Codex."
---
...
Invocacao original no Claude: `/jc:docs`

Invocacao equivalente no Codex: `$jc-docs`
...
> No Codex, use o script empacotado globalmente em `${CODEX_HOME:-$HOME/.codex}/skills/jc-docs/scripts/confluence.js`.
...
```bash
node ${CODEX_HOME:-$HOME/.codex}/skills/jc-docs/scripts/confluence.js search --cql 'space = "DT" AND title = "API SIJ"'
```
...
```
Para sincronizar com o Confluence preciso das suas credenciais Atlassian (são salvas globalmente em `$CODEX_HOME/atlassian.json`, com fallback para `~/.codex/atlassian.json` e compatibilidade com `~/.claude/atlassian.json` — nunca no repositório).
...
```bash
node ${CODEX_HOME:-$HOME/.codex}/skills/jc-docs/scripts/confluence.js setup --email <EMAIL> --token <TOKEN>
```
...
```bash
node ${CODEX_HOME:-$HOME/.codex}/skills/jc-docs/scripts/confluence.js get --page-id <PAGE_ID>
```
...
```bash
node ${CODEX_HOME:-$HOME/.codex}/skills/jc-docs/scripts/confluence.js search --cql 'space = "DT" AND title = "Nome da Página"'
```
...
**Se a página existir:**
```bash
node ${CODEX_HOME:-$HOME/.codex}/skills/jc-docs/scripts/confluence.js update --page-id <ID> --title "Nome da Página" --body-file docs/{arquivo}.xhtml
```

**Se a página não existir** → perguntar ao usuário em qual seção/página pai criar:
```bash
node ${CODEX_HOME:-$HOME/.codex}/skills/jc-docs/scripts/confluence.js create --title "Nome da Página" --body-file docs/{arquivo}.xhtml --parent-id <PARENT_ID>
```
...
- **Auth**: lido de `$CODEX_HOME/atlassian.json`, com fallback para `~/.codex/atlassian.json` e compatibilidade com `~/.claude/atlassian.json` (salvo via `confluence.js setup`) ou env vars `ATLASSIAN_EMAIL` / `ATLASSIAN_API_TOKEN`
- **Nunca salvar credenciais no repositório do projeto**
- **Script**: `${CODEX_HOME:-$HOME/.codex}/skills/jc-docs/scripts/confluence.js`
