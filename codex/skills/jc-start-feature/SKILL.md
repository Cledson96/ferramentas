---
name: jc-start-feature
description: "Inicia uma feature — busca card Jira, cria branch com o ID, carrega contexto e sugere plano de implementacao. Use quando o usuario mencionar /jc:start-feature, pedir esse workflow da JusCash ou quiser o comportamento equivalente do plugin jc no Codex."
---
...
Invocacao original no Claude: `/jc:start-feature`

Invocacao equivalente no Codex: `$jc-start-feature`
...
Verificar se existe `.context/project-context.md` na raiz do projeto:
- **Se existir**: ler e usar como contexto — mencionar ao usuário que o contexto foi carregado
- **Se não existir `.context/project-context.md`**: executar `$project-context` automaticamente com `ensure` para gerar (seguir regra de autonomia)
- **Se existir `.context/` mas o contexto estiver stale**: executar `$project-context` com `ensure` para atualizar
- **Se existir contexto leve e artefatos Repomix**: usar primeiro `.context/project-context.md` e recorrer ao restante apenas se precisar de detalhe

Se existir `.context/` gerado pelo workflow de contexto, priorizar:
- `.context/project-context.md` — visão geral, comandos, convenções e navegação
- `.context/repomix/token-count-tree.txt` — identificar arquivos caros antes de abrir
- `.context/repomix/repomix-compressed.xml` — referência expandida e mais barata quando precisar de mais detalhe
